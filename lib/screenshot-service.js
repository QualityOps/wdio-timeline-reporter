...
class ScreenshotService {
    ...
    after() {
        ...
        if (resize) {
            console.log(`TIMELINE:ScreenshotService: Attempting to resize ${this.screenshotList.length} images`)
            quality = (Number.isInteger(quality) && quality > 0 && quality <= 100) ? Math.round(quality) : 70;
            reductionRatio = (Number.isInteger(reductionRatio) && reductionRatio > 0 && reductionRatio <= 5) ? Math.round(reductionRatio) : 1;
            const promises = this.screenshotList.map(filePath =>
                Promise.resolve(waitForFileExistsAndResize(filePath, quality, reductionRatio)));
