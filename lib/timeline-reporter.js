...
class TimelineReporter extends EventEmitter {
  constructor(baseReporter, config, options = {}) {
    ...
    this.on('runner:screenshot', function(runner) {
      ...
      test && test.screenshots && test.screenshots.push(filePath);
    });
    ...
    this.on('end', function() {
      try {
        ...
        if (!fileName.endsWith('html')) {
          console.warn(
            `TIMELINE REPORTER: Report file name should end with html. Your file name is "${fileName}"`
          );
        ...
        console.warn(
          `TIMELINE REPORTER: Could not create timeline report. See error below\n\n${e}`
        );
      }
    });
  }
  ...
  generateTestResults() {
    ...
    const passed = runners.reduce(
      (accumulator, cid) => this.results[cid].passing + accumulator,
      0
    );
    const failed = runners.reduce(
      (accumulator, cid) => this.results[cid].failing + accumulator,
      0
    );
    const skipped = runners.reduce(
      (accumulator, cid) => this.results[cid].pending + accumulator,
      0
    );
    ...
    for (const cid of runners) {
      ...
      for (const specId of Object.keys(runnerInfo.specs)) {
        ...
        for (const suiteName of Object.keys(specInfo.suites)) {
          ...
          if (
            !suiteInfo.uid.includes('before all') &&
  ...
  resolveScreenshotPath(filename) {
    ...
    const fileNamePrependedWithScreenshotPath = path.resolve(
  ...
  getReporterOptions() {
    ...
    return deepMerge(defaultOptions, this.options.timelineReporter || {});
  }
